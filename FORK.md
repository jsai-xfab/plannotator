...
| 6 | **Guided Review plans against source only.** Generated files are no longer sent to the guide agent, so no chapter is spent on a lockfile. | `packages/server/review.ts` |
| 7 | **The review skill teaches the loop.** Classify each thread, say how you read it, edit for a change request, answer a question without touching files, resolve only what you changed. | `apps/skills/core/plannotator-review/SKILL.md` |
| 8 | **Code Tour stops render diagrams.** A stop's detail goes through the full markdown renderer, so a ```mermaid fence renders as a picture — plus code blocks and tables. The tour prompt asks for a diagram when a stop is about structure or sequence, drawn from real names in the diff. | `packages/ui/components/RenderedMarkdown.tsx`, `TourStopCard.tsx`, `packages/server/tour/tour-review.ts` |
...
| 17 | **A chapter's diffs collapse into a file list.** One control per chapter turns every diff into a single row: path, counts, and the description the guide already wrote for that file. Rows group by folder, with each folder's file count and totals, so the reader sees how much of the chapter is one package. Measured on a 16-file chapter: 10331px of scrolling becomes 1694px. Folder headers render only while collapsed. | `groupFilesByFolder.ts`, `GuideSectionCard.tsx`, `GuideFileCard.tsx` |
...
| **Versions panel** (document review) | The `Versions` tab, `CURRENT PLAN / Version n of m`, `COMPARE AGAINST v1..vn`, and the inline redline it renders. `packages/ui/components/sidebar/VersionBrowser.tsx` and `usePlanDiff.ts`, both untouched by this fork. **This is the answer to "show me only what the agent changed since I last read it."** This fork briefly built a second mechanism for the same job before finding this one; see "Removed from this fork". |
...
| **Review rounds** (`round-store.ts`, `since-review.ts`, and the `recordReviewRound` hook on `/api/feedback`) | Built, then deleted. It recorded every file's git blob id when the reviewer sent feedback, so a later pass could show only the agent's edits. Two problems. The reviewer asked for this about **documents**, and upstream's Versions panel already did it. For **code** a commit answers the same question, so the request was never made. It also never gained a view: 343 lines nothing read. |
