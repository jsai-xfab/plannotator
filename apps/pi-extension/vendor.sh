...
for f in feedback-templates project favicon code-file annotatable annotation-threads external-annotation agent-jobs agent-terminal source-save open-in-apps diff-paths diff-files source-lines thread-resolution guide-groups guide guide-format guide-viewer-manifest compress crypto; do
